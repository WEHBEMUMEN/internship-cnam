...
export class BasisPlot {
    ...
    drawCalculusVectors(xi, p, knots, points, weights, CurveEngine) {
        const { width, height } = this.canvas;
        // Bounding the native quotient analytical rule payload
        const res = CurveEngine.evaluateFirstDerivative(xi, p, knots, points, weights);
        
        // Translate geometric coordinate mapping universally exact
        const bx = width * (0.1 + res.val.x * 0.8);
        const by = height * (0.1 + res.val.y * 0.8);
        
        // Derivative fractional scaling natively to purely structural pixel density
        const dx = res.deriv.x * width * 0.08;
        const dy = res.deriv.y * height * 0.08;

        // Draw Tangent Kinematic Vector Field Map
        this.ctx.beginPath();
        this.ctx.moveTo(bx, by);
        this.ctx.lineTo(bx + dx, by + dy);
        this.ctx.strokeStyle = '#ef4444'; // Red Vector Analytical Path Limits
        this.ctx.lineWidth = 3;
        this.ctx.stroke();

        // Highlighting Origin Stress Point physically
        this.ctx.beginPath();
