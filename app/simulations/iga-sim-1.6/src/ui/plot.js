...
export class BasisPlot {
    ...
    drawFEMCurve(deflection, visualScale = 0.05) {
        const { width, height } = this.canvas;
        const numNodes = deflection.length;
        ...
        for (let i = 0; i < numNodes; i++) {
            const u = deflection[i];
            const x_norm = i / (numNodes - 1);
            const wy = 0.5 - u * visualScale; 
            const pt = this.worldToScreen(x_norm, wy);
            
            if (i === 0) this.ctx.moveTo(pt.x, pt.y);
            else this.ctx.lineTo(pt.x, pt.y);

            this.ctx.fillStyle = '#10b981';
            this.ctx.fillRect(pt.x - 3, pt.y - 3, 6, 6);
        }
        this.ctx.stroke();
        this.ctx.setLineDash([]);
    }
