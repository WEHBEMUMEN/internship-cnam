...
class DEIMEngine {
    ...
    train(forceSnapshots, m, kf = m) {
        ...
        this.history = [{
            step: 1,
            residual: Array.from(r0),
            point: maxIdx,
            maxVal: maxVal
        }];

        // Step 2: Greedy selection using all m modes
        for (let l = 1; l < this.m; l++) {
            const ul = new Float64Array(N);
            for (let i = 0; i < N; i++) ul[i] = U_greedy.get(i, l);
            ...
            const c = DEIMEngine._solveLinear(Psub, rhs);
            ...
            const rMag = new Float64Array(N);
            for (let i = 0; i < N; i++) {
                rMag[i] = Math.abs(r[i]);
                if (!indices.includes(i) && rMag[i] > maxR) {
                    maxR = rMag[i];
                    bestIdx = i;
                }
            }
