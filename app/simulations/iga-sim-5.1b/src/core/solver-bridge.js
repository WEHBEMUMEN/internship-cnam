...
class SolverBridge {
    ...
    solveLinearStatic(patch, Tx = 100.0) {
        ...
        try {
            // Apply physical conditions logic from Phase 2.2
            const symmetryBcs = [];
            for (let j = 0; j < nV; j++) symmetryBcs.push({ i: 0, j: j, axis: 'y', value: 0 }); // Bottom Edge
            for (let j = 0; j < nU; j++) symmetryBcs.push({ i: nU - 1, j: j, axis: 'x', value: 0 }); // Left Edge
            
            const tractionLoads = [];
            // Right edge (Tension) is handled by the solver's nodal traction method
            
            const u = this.solver.solve(patch, symmetryBcs, []);
