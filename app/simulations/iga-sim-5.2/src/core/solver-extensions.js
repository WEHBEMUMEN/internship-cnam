...
class ECSWSolver extends window.IGA2DSolver {
    ...
    /**
     * Overrides base traction calculation to use uniform tension at the beam tip (u=1).
     */
    calculateNodalTraction(patch, loadValue, edge = 'right') {
        ...
        const uParam = 1.0 - 1e-6; // Tip of the beam

        if (edge === 'right') {
            // Loop through boundary elements in the V direction (vertical edge)
            for (let e = 0; e < uniqueV.length - 1; e++) {
                const vMin = uniqueV[e], vMax = uniqueV[e+1];
                if (vMax - vMin < 1e-9) continue;

                for (let qv = 0; qv < gPoints.length; qv++) {
                    const v = ((vMax - vMin) * gPoints[qv] + (vMax + vMin)) / 2;
                    const gw = gWeights[qv] * (vMax - vMin) / 2;

                    const deriv = this.engine.getSurfaceDerivatives(patch, uParam, v);
                    const tangent = deriv.dV; // Tangent along the vertical edge
                    const detJ_1D = Math.sqrt(tangent.x**2 + tangent.y**2);
                    if (detJ_1D < 1e-12) continue;

                    // Uniform axial tension (Traction vector = [Tx, 0])
                    const tx = loadValue;
                    const ty = 0;

                    // NURBS Rational Basis along V
                    let W_1D = 0;
                    for (let b = 0; b < nV; b++) {
                        const Mb = this.engine.basis1D(b, q, V, v);
                        W_1D += Mb * weights[nU-1][b];
                    }
                    ...
                    for (let b = 0; b < nV; b++) {
                        const Mb = this.engine.basis1D(b, q, V, v);
                        if (Mb === 0) continue;
                        const Rb = (Mb * weights[nU-1][b]) / W_1D;
                        
                        // Map to global DOF index (Node index i=nU-1, j=b)
                        const idx = ((nU - 1) * nV + b) * 2;
                        forces[idx]     += Rb * tx * detJ_1D * gw;
                        forces[idx + 1] += Rb * ty * detJ_1D * gw;
                    }
                }
            }
        }
