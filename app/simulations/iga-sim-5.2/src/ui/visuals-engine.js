...
class VisualsEngine {
    constructor(containerId, nurbs, solver) {
        ...
        this.camera.position.set(5, 0, 15); // Centered for 10.0 length beam
    ...
    renderBCs(patch) {
        ...
        // 1. Clamped BC (Fixed on Left Edge: i=0)
        const clampMaterial = new THREE.MeshPhongMaterial({ color: 0xef4444, transparent: true, opacity: 0.9 });
        const clampGeo = new THREE.ConeGeometry(0.12, 0.25, 4);
        clampGeo.rotateX(Math.PI / 2);
        clampGeo.rotateZ(Math.PI / 2); // Pointing towards the beam

        for (let j = 0; j < nV; j++) {
            const clamp = new THREE.Mesh(clampGeo, clampMaterial);
            clamp.position.set(cp[0][j].x - 0.15, cp[0][j].y, cp[0][j].z);
            this.bcGroup.add(clamp);
        }

        // 2. Loads (Tension Arrows on Right Edge: i=nU-1)
        const arrowDir = new THREE.Vector3(1, 0, 0);
        const arrowColor = 0x3b82f6; 
        const arrowLength = 0.8;

        for (let j = 0; j < nV; j++) {
            const cp_node = cp[nU - 1][j];
            const origin = new THREE.Vector3(cp_node.x, cp_node.y, cp_node.z);
            const arrow = new THREE.ArrowHelper(arrowDir, origin, arrowLength, arrowColor, 0.2, 0.1);
