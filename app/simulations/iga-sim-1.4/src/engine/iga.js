...
export class BasisFunctions {
    ...
    static evaluateDerivative(i, p, xi, U) {
        if (p === 0) return 0;
        let p1 = 0, p2 = 0;
        const den1 = U[i+p] - U[i];
        if (den1 > 0) p1 = (p / den1) * this.evaluate(i, p-1, xi, U);
        const den2 = U[i+p+1] - U[i+1];
        if (den2 > 0) p2 = (p / den2) * this.evaluate(i+1, p-1, xi, U);
        return p1 - p2;
    }
...
export class Curve {
    ...
    static evaluateFirstDerivative(xi, p, U, points, weights) {
        const n = points.length;
        let A = {x:0, y:0};
        let w = 0;
        let Ad = {x:0, y:0};
        ...
        for (let i=0; i<n; i++) {
            const N = BasisFunctions.evaluate(i, p, xi, U);
            const Nd = BasisFunctions.evaluateDerivative(i, p, xi, U);
            
            A.x += N * weights[i] * points[i].x;
            A.y += N * weights[i] * points[i].y;
            w += N * weights[i];
            
            Ad.x += Nd * weights[i] * points[i].x;
            Ad.y += Nd * weights[i] * points[i].y;
            wd += Nd * weights[i];
        }
